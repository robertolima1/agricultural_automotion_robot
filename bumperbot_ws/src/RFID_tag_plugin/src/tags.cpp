...
public:
    ...
    void spawn_fixtures(Spawner *spawn)
    {
        ...
        while (idx < number_fixtures)
        {
            ...
            fix.request.model_xml = make_box_sdf(fixtures[idx].name, 5.5, 0.25, 0.08, /*static=*/true);
    ...
    static std::string make_box_sdf(const std::string &model_name,
                                    ...
                                    bool is_static = true)
    {
        ...
        ss << "<?xml version=\"1.0\"?>\n"
           ...
           << "    <static>" << (is_static ? "true" : "false") << "</static>\n"
           ...
           << "</size></box></geometry>\n"
           << "        <material>\n"
        << "          <ambient>0.36 0.25 0.20 1</ambient>\n"
        << "          <diffuse>0.36 0.25 0.20 1</diffuse>\n"
    ...
    static std::string make_cylinder_sdf(const std::string& model_name,
                                     double radius, double length,
                                     double mass = 0.01,
                                     std::array<double,4> rgba = {0.0, 0.6, 0.0, 1.0},
                                     bool is_static = true)
{
    // Inércia de cilindro sólido alinhado ao eixo Z:
    // Ixx = Iyy = (1/12) * m * (3r^2 + L^2)
    // Izz = (1/2) * m * r^2
    const double Ixx = (1.0/12.0) * mass * (3.0*radius*radius + length*length);
    const double Iyy = Ixx;
    const double Izz = 0.5 * mass * radius * radius;
    ...
    // Inércia é opcional para modelos estáticos, mas não atrapalha:
    ss << "      <inertial>\n"
       << "        <mass>" << mass << "</mass>\n"
       << "        <inertia>\n"
       << "          <ixx>" << Ixx << "</ixx>\n"
    ...
    void spawn_tags(float x, float y, float z, Spawner *tag_spawner, int *tag_count)
    {
        ...
        while (idx <= tags_for_fix)
        {
            ...
            tags[*tag_count].x = (x + idx) - 2.5;
            tags[*tag_count].y = y ;
            tags[*tag_count].z = z + 0.15 ;
            ...
            tag.request.model_xml = make_cylinder_sdf(tags[*tag_count].name, 0.05, 0.2);
            tag_spawner->spawn_obj(tag);
            cout << "tag " << tags[*tag_count].name << " is spawning " << endl;
            ROS_INFO("AQUI %s",path_white_tag.c_str());
