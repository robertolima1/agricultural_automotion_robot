...
public:
    ...
    double trilateration_euclidian(float distance_x1, float distance_x2, float distance_x3, Point p1, Point p2, Point p3)
    {
        ...
        float A = 2 * (p2.x - p1.x);
        float B = 2 * (p2.y - p1.y);
        float C = distance_x1 * distance_x1 - distance_x2 * distance_x2 - p1.x * p1.x + p2.x * p2.x - p1.y * p1.y + p2.y * p2.y;
        float D = 2 * (p3.x - p2.x);
        float E = 2 * (p3.y - p2.y);
    ...
    void antenna_start(Spawner *spawn)
    {
        ...
        while (node.ok())
        {
            try
            {
                ...
                ros::NodeHandle nh;
                boost::shared_ptr<sensor_msgs::Imu const> msg;
                msg = ros::topic::waitForMessage<sensor_msgs::Imu>("/imu_ekf", nh);
                // aplicar triangulação.
                ROS_INFO("IMU - Orientation -> w: %.3f x: %.3f y: %.3f z: %.3f",
                         msg->orientation.w,
                         msg->orientation.x,
                         msg->orientation.y,
                         msg->orientation.z);
                for (int i = 1; i <= 3; i++)
                {
                    ...
                    for (int idx = 1; idx < MAX_TAGS; idx++)
                    {
                        ...
                        printf("\n[INFO] Antena ID:  %d:\n", antenna_id);
                        printf("\n[INFO] Target Frame:  %s:\n", target_frame.c_str());
                        printf("\n[INFO] Transform recebido para tag %d:\n", idx);
                        tf = tfBuffer.lookupTransform(target_frame, source_frame + to_string(idx), ros::Time(0));
                        // printf("\n=== [TAG %d] Buscando transform de '%s' para '%s' ===\n", idx + 1, target_frame.c_str(), source_frame.c_str());
